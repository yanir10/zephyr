...
#define DC_CTRL_RGN_OP_MASK          GENMASK(11, 9) /* d-cache region operation mask */

#define MMU_BUILD_PAE_MASK BIT(12) /* physical address extension enable mask */

#define SLC_CTRL_DISABLE_MASK BIT(0)         /* SLC disable mask */
#define SLC_CTRL_IM_MASK      BIT(6)         /* SLC invalidate mode mask  */
#define SLC_CTRL_BS_MASK      BIT(8)         /* SLC busy status mask */
#define SLC_CTRL_RGN_OP_MASK  GENMASK(11, 9) /* SLC region operation mask */
...
static bool pae_exists(void)
{
	...
	return 1 == FIELD_GET(MMU_BUILD_PAE_MASK, bcr);
}
...
static void slc_flush_region(void *start_addr_ptr, size_t size)
{
	...
	K_SPINLOCK(&slc_lock) {
		 ...
		 */
		end_addr = start_addr + size + CONFIG_SLC_LINE_SIZE - 1;

		z_arc_v2_aux_reg_write(_ARC_V2_SLC_RGN_END, end_addr);
		z_arc_v2_aux_reg_write(_ARC_V2_SLC_RGN_START, start_addr);

		/* Make sure "busy" bit reports correct status, see STAR 9001165532 */
		z_arc_v2_aux_reg_read(_ARC_V2_SLC_CTRL);
		while (z_arc_v2_aux_reg_read(_ARC_V2_SLC_CTRL) & SLC_CTRL_BS_MASK) {
			/* Do Nothing */
		}
	}
}
...
static void slc_invalidate_region(void *start_addr_ptr, size_t size)
{
	...
	K_SPINLOCK(&slc_lock) {
		...
		ctrl &= ~SLC_CTRL_IM_MASK;

		ctrl &= ~SLC_CTRL_RGN_OP_MASK;
		ctrl |= FIELD_PREP(SLC_CTRL_RGN_OP_MASK, 0x1);
...
static void dcache_invalidate_region(void *start_addr_ptr, size_t size)
{
	...
	ctrl &= ~DC_CTRL_IM_MASK;

	ctrl &= ~DC_CTRL_RGN_OP_MASK;
	ctrl |= FIELD_PREP(DC_CTRL_RGN_OP_MASK, 0x1);
...
#else /* CONFIG_DCACHE_REGION_OPERATIONS */
...
#endif /* CONFIG_DCACHE_REGION_OPERATIONS */
